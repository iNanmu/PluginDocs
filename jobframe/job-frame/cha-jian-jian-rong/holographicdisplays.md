...
可为对象 **创建全息内容** 显示，同时可设置全息对不同玩家的可见度设置(需要 **ProtocolLib** 支持)，使用该功能建议安装 **ProtocolLib** 这样子才可以获得最好体验，可通过 [动作脚本(Hologram)](../gong-zuo-dui-xiang/dui-xiang-dong-zuo-jiao-ben/hologram.md) 进行操作

### 全息配置

该位置的配置直接写到工作组配置上即可，具体格式如下

```yaml
hologram:
  #生成的高度
  high: 0.5
  #每秒更新一次,如果关闭的话需要通过 hologram update all 脚本进行更新
  update: true
  #公共组(会根据对象名的变动而变动)
  default:
    #对象名判断
    #会根据对象名的变动而变动,提前需要对象名变动是所有玩家变动,即使用
    #job:object name all 蓝水晶矿根 (ALL 类型, SELF 是不会变动的)
    "蓝水晶矿根":
      - "§f[ §6蓝水晶矿物 §f]"
  ...
  #除了上方的公共组设置的全息内容,还可以设置非公共组上的全息内容
  #需要使用 hologram send 动作脚本发送显示
  "test1":
    - "这是一个非公共组的全息设置"
  #"test2":
  #"test3":
